defmodule Pooly.PoolServer do
	...
	def handle_call({:checkout, block}, {from_pid, ref} = from, state) do
		...
		case workers do
		   [worker | rest] ->
		    ...
		    [] when max_overflow > 0 and overflow < max_overflow ->
		    	...
		    	{:reply, new_worker, %{state | overflow: overflow + 1}}
		    [] when block == true ->
		    	ref = Process.monitor(from_pid)
	...
	#handling consumer crash
	def handle_info({:DOWN, ref, _, _, _}, state = %{monitors: monitors, workers: workers}) do
		case :ets.match(monitors, {:"$1", ref}) do
			[[pid]] ->
				true = :ets.delete(monitors, pid)
				new_state = %{state | workers: [pid | workers]} #returns worker to the pool
				{:noreply, new_state}
			[[]] ->
				{:noreply, state}
		end
